...
| Param | Required | Notes |
|---|---|---|
| `grant_type` | yes | **`urn:ietf:params:oauth:grant-type:device_code`** (verbatim; the exact value `@azure/msal-node` sends). Registered as the dispatch key in `GRANT_HANDLERS`. |
...
1. `grant_type` matches the URN dispatch key. Any additional poll parameters (`scope`, `client_info`, correlation/telemetry/library fields) are **ignored**; a `scope` param, if sent, is discarded and never alters the granted scopes (those come solely from the stored device-code row).
...
|---|---|---|---|
| `grant_type` value | `device_code` short form vs the RFC URN | `urn:ietf:params:oauth:grant-type:device_code` | Exactly what `@azure/msal-node` sends; RFC 8628 §3.4 mandates it. |
